...
public class MinAbsoluteSumDiff {
    public int minAbsoluteSumDiff(int[] nums1, int[] nums2) {
        ...
        for (int i = 0; i < nums1.length; i++){
            ...
            int left = 0, right = nums3.length - 1;
            int j;
            while (left < right - 1){
                ...
                if (nums3[mid] == nums2[i]){
                    left = right = mid;
                    break;
                }else if (nums3[mid] < nums2[i]){
                    left = mid;
                }else{
                    right = mid;
                }
            }
            if (left < nums3.length - 1){
                j = Math.min(Math.abs(nums3[left] - nums2[i]), Math.abs(nums3[left + 1] - nums2[i]));
            }else{
                j = Math.abs(nums3[left] - nums2[i]);
            }
            if (temp - j > max){
                max = temp - j;
            }
        ...
        return Math.abs(res - max + mod) % mod;
    }
}
